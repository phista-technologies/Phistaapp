...
class WalletControllerOwner extends GetxController {
  Rx<TextEditingController> amountController = TextEditingController().obs;
  Rx<TextEditingController> withdrawalAmountController = TextEditingController().obs;
  Rx<TextEditingController> noteController = TextEditingController(
    text: "Withdrawal",
  ).obs;
        ...
        await Stripe.instance.initPaymentSheet(
          paymentSheetParameters: SetupPaymentSheetParameters(
            ...
            googlePay: PaymentSheetGooglePay(
              ...
              testEnv: paymentModel.value.strip?.isSandbox == true ? true : false,
              currencyCode: "CAD",
            ),
  ...
  mercadoPagoMakePayment({
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final bool isDone = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => MercadoPagoScreen(initialURl: data['init_point']),
        ),
      );
  ...
  flutterWaveInitiatePayment({
    ...
    if (response.statusCode == 200) {
      ...
      final bool isDone = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => MercadoPagoScreen(initialURl: data['data']['link']),
        ),
      );
  ...
  Future<String> createPaymentLink({required var amount}) async {
    ...
    final url = Uri.parse(
      paymentModel.value.midtrans!.isSandbox! ? 'https://api.sandbox.midtrans.com/v1/payment-links' : 'https://api.midtrans.com/v1/payment-links',
    );
    ...
    final response = await http.post(
      ...
      headers: {
        ...
        'Content-Type': 'application/json',
        'Authorization': generateBasicAuthHeader(
          paymentModel.value.midtrans!.serverKey!,
        ),
      },
      body: jsonEncode({
        'transaction_details': {
